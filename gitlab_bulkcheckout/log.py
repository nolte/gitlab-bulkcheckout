...
LOG_FORMATS = {"DEBUG": u"%(levelname)s %(name)s: %(message)s", "INFO": u"%(levelname)s: %(message)s"}
...
def configure_logger(stream_level="DEBUG", debug_file=None):
    ...
    """
    # Set up 'gitprojectmanager' logger
    logger = logging.getLogger("gitlab_bulkcheckout")
